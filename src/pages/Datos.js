...
const Datos = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    fetch(url, requestOptions).then((res) => setData([...data, res]));

    setCambio(true);
    console.log(cambio);
  };
  /* to={`/datos/${JSON.stringify(genero)}` */
  if (cambio) return <Redirect to={`/horoscopo/${JSON.stringify(form)}`} />;
